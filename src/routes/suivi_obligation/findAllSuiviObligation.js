...
module.exports = (app) => {
    app.get("/api/suivi_obligation", auth(1, getUserRole), async (req, res) => {
      try {
        // Récupération de toutes les suivi_obligation
        const suivi_obligation = await models.suivi_obligation.findAll({
          order: ["id_suivi"],
        });
        ...
        const message =
          "Une erreur est survenue lors de la récupération de la liste des suivi_obligation";
