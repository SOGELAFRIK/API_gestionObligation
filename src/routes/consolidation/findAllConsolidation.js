...
module.exports = (app) => {
  app.get("/api/consolidation", auth(1, getUserRole), async (req, res) => {
    try {
      // Récupération de toutes les consolidation
      const consolidation = await models.consolidation.findAll({
        order: ["id_consolidation"],
      });
      ...
      const message =
        "Une erreur est survenue lors de la récupération de la liste des consolidation";
